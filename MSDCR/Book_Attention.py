...
class Attention_book(nn.Module):

    def __init__(self, latent_dim, d_k=16, d_v=16, n_heads=1, is_layer_norm=True, attn_dropout=0.1):
        super(Attention_book, self).__init__()
    ...
    def scaled_dot_product_attention(self, Q, K, V):
        '''
        :param Q: (*, max_q_words, n_heads, latent_dim)
        :param K: (*, max_k_words, n_heads, latent_dim)
        :param V: (*, max_v_words, n_heads, latent_dim)
        :param episilon:
        :return:
        '''
        ...
        V_att = Q_K_score.bmm(V)  # (*, max_q_words, latent_dim)
        return V_att
    ...
    def forward(self, Q, K, V):
        '''
        :param Q: (batch_size, max_q_words, latent_dim)
        :param K: (batch_size, max_k_words, latent_dim)
        :param V: (batch_size, max_v_words, latent_dim)
        :return:  output: (batch_size, max_q_words, latent_dim)  same size as Q
        '''
