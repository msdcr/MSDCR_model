...
from MV_DNN.NCF import Cosine_Score
from MV_DNN.dataprocessing import item_Feature,validate_test_sample

device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

class MV_DNN(torch.nn.Module):
    def __init__(self, input_mov_col, input_bok_col, latent_dim, latent_media_dim, attn_dropout, input_dim_user,input_mov_item,input_bok_item, latent_item_dim, batch_size):
        super(MV_DNN, self).__init__()
        ...
        self.Embedding_user = torch.nn.Embedding(self.input_dim_user, (self.input_mov_col + self.input_bok_col) * self.latent_item_dim)
        ...
        self.Linear_user = torch.nn.Sequential(
            nn.Linear((self.input_mov_col + self.input_bok_col) * self.latent_item_dim, self.latent_media_dim),
            nn.Dropout(self.attn_dropout),
            nn.Tanh(),
            nn.Linear(self.latent_media_dim, self.latent_media_dim)
        )
        self.Linear_mov = torch.nn.Sequential(
            nn.Linear(self.input_mov_col * self.latent_item_dim, self.latent_media_dim),
            nn.Dropout(self.attn_dropout),
            nn.Tanh(),
            nn.Linear(self.latent_media_dim, self.latent_media_dim)
        )
        self.Linear_bok = torch.nn.Sequential(
            nn.Linear(self.input_bok_col * self.latent_item_dim, self.latent_media_dim),
            nn.Dropout(self.attn_dropout),
            nn.Tanh(),
            nn.Linear(self.latent_media_dim, self.latent_media_dim)
        )
        self.Cosine_Score = Cosine_Score(self.latent_media_dim, self.latent_dim)
    ...
    def forward(self, batch_size, u, i_mov, X_movie, i_bok, X_book):
        ...
        user_embedding = self.Linear_user(user_embedding)
        ...
        rating_mov_i = self.Cosine_Score(user_embedding, e_mov_i)
        rating_bok_i = self.Cosine_Score(user_embedding, e_bok_i)
...
class MV_DNN_runner(object):
    def __init__(self,input_mov_col, input_bok_col,input_dim_user,input_mov_item,input_bok_item,config):
        self.config = config
        self.sigmoid = torch.nn.Sigmoid()
        self.MV_DNN = MV_DNN(input_mov_col=input_mov_col, input_bok_col=input_bok_col,latent_dim=config['latent_dim'], latent_media_dim=config['latent_media_dim'], attn_dropout=config['attn_dropout']
                         ,input_dim_user=input_dim_user,input_mov_item = input_mov_item,input_bok_item = input_bok_item,latent_item_dim=config['latent_item_dim'], batch_size=self.config['batch_size'])
        self.MV_DNN.to(device)

        self.opt_mov = torch.optim.Adam([
                                       {'params': self.MV_DNN.Cosine_Score.parameters()},
                                       {'params': self.MV_DNN.Linear_mov.parameters()},
                                       {'params': self.MV_DNN.Linear_user.parameters()},
                                       {'params': self.MV_DNN.Embedding_mov.parameters()},
                                       {'params': self.MV_DNN.Embedding_user.parameters()}], lr=config['lr'],  weight_decay=0.001)

        self.opt_bok = torch.optim.Adam([{'params': self.MV_DNN.Cosine_Score.parameters()},
                                       {'params': self.MV_DNN.Linear_bok.parameters()},
                                       {'params': self.MV_DNN.Linear_user.parameters()},
                                       {'params': self.MV_DNN.Embedding_bok.parameters()},
                                       {'params': self.MV_DNN.Embedding_user.parameters()}], lr=config['lr'], weight_decay=0.001)
    ...
    def train_single_batch(self, batch_train):
        ...
        rating_mov_i, rating_bok_i = self.MV_DNN(batch_size, U_P, I_mov_P, M_mov_P, I_bok_P, B_bok_P)

        rating_mov_j, rating_bok_j = self.MV_DNN(batch_size, U_P, I_mov_N, M_mov_P, I_bok_N, B_bok_P)
    ...
    def train_single_epoch(self, train, config, epoch_id):
        ...
        with open('../parameters/MV_DNN_epoch_loss.txt', 'a+') as f:
