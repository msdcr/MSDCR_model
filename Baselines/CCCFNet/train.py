...
import torch
import pickle
from Model.dataprocessing import train_sample, validate_test_sample
from CCCFNet.model import CCCFNet_runner
...
def SampleTrain():
    ...
    input_bok_col = 4
    input_dim_user = 2111
    input_mov_item = 21948  # the number of item features
    input_bok_item = 14298
    input_mov = 21638  # the number of items
    input_bok = 19473

    model = CCCFNet_runner(input_mov_col, input_bok_col, input_mov, input_bok, input_dim_user, input_mov_item, input_bok_item, config)
    ...
    with open('../data/mov_bok/train(12).pkl', 'rb') as f:
        ...
        for epoch in range(config['epoch']):
            ...
            if loss_epoch_mean < min_loss:
                ...
                print("save model")
                torch.save(model.CCCFNet.state_dict(), '../parameters/' + 'CCCFNet_best' + '.pth')
                if l < 2:
                    i += 1
                    if i == 5:
                        break
    ...
    model.CCCFNet.load_state_dict(torch.load('../parameters/' + 'CCCFNet_best' + '.pth'))
    print('Start test !!!')
